...
Test(ch05_test_suite, test_curl_fetch_mainnet) {
  ...
  vector<uint8_t> d(64);
  bool retval = Tx::fetch_tx(tx_id, false, d);
  ...
  cr_assert(eq(u32, sizeof(expected)/sizeof(expected[0]), d.size()));
  cr_assert(eq(i32, retval, 0));
  for (size_t i = 0; i < sizeof(expected)/sizeof(expected[0]); ++i) {
    cr_assert(eq(u8, expected[i], d[i]));
  }
}
...
Test(ch05_test_suite, test_curl_fetch_testnet) {
  ...
  vector<uint8_t> d(64);
  bool retval = Tx::fetch_tx(tx_id, true, d);
